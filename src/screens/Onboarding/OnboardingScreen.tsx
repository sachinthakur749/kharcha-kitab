...
// Safe lazy-load for native-only notification listener
...
try {
  ...
  // Not available in Expo Go - that's fine
}
...
export const OnboardingScreen: React.FC = () => {
  ...
  const steps = [
    ...
    },
    {
      ...
      icon: '🔔',
      action: 'Enable Notification Access',
    },
    {
      title: 'All Set!',
      description:
  ...
  const handleAction = async () => {
    ...
    try {
      if (currentStep === 1) {
        // SMS Permission
        // SmsService.requestPermissions already has Platform checks
        ...
        if (granted) {
          ...
          Alert.alert(
            'Action Needed',
            'Please grant SMS permissions in your phone settings to enable automated tracking. (Note: Features require a native build)'
          );
        }
        ...
        if (!RNAndroidNotificationListener || !RNAndroidNotificationListener.getPermissionStatus) {
          Alert.alert(
            ...
            'Notification tracking is only available in a native build. In Expo Go, you can skip this step.'
          );
          return;
        }
        ...
        if (status !== 'authorized') {
          ...
          Alert.alert('Check Settings', 'Please enable access for "Kharcha Kitab" in the settings screen that just opened.');
        } else {
          Alert.alert('Success', 'Notification access already granted!');
          handleNext();
        }
      }
    } catch (error) {
      console.error('[Onboarding] Error handling action:', error);
